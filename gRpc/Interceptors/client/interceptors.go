...
func (s *wrappedClientStream) SendMsg(m any) error {
...
func loggingStreamingInterceptor(
	...
	c := streamDurationContextValue{startTime: time.Now()}
	ctxWithTimer := context.WithValue(ctx, streamDurationContextKey{}, c)

	stream, err := streamer(ctxWithTimer, desc, cc, method, opts...)
